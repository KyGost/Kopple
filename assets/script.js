...
const controlFilter = () => {
	new Dialog({
		...
		doneText: "Filter",
		inputs: [
			{ id: "blacklist", label: "Blacklist", type: "checkbox" },
			{ type: "hidden" },
		].concat(
			[
				...Object.keys(Store.knowledgeBase)
					.map((address) =>
						Store.knowledgeBase[address].feed?.map((post) => post.topics).flat()
					)
					.flat()
					.reduce(
						(accumulative, current) =>
							accumulative.set(current, (accumulative.get(current) || 0) + 1),
						new Map()
					),
			]
				.sort((a, b) => b[1] - a[1])
		...
		doneClick: () => {
			...
			State.hashSet(
				"FILTER",
				`(${isBlacklist ? "?!" : ""}${Array.from(
					document.querySelectorAll(".dialog .topic")
				)
...
const menuFollow = () => {
	let extraInfo = Utilities.newElement("found", "p");
	new Dialog({
		title: "Follow User",
		...
		inputs: [
			{
				...
				type: "text",
				placeholder:
					"833d719039176d2803dcb76576684864953e2550760209b76183054841471fda",
				events: {
					keyup: (event) => {
...
const onStart = () => {
	if (State.hash.POST) {
		let { POST, ...hash } = {
			...State.hash,
			PAGE: { POST: Object.keys(State.hash.POST)[0] },
		};
		State.hash = hash;
	}
	...
	fetch(`hyper://${Setting.profileDrive}`, { method: "HEAD" }).then(
		(response) => {
			if (!/.*PUT.*/.test(response.headers.get("Allow") || "PUT"))
	...
	Store.loadFiles().then(() => {
		...
		document
			.querySelector("#menuSelf")
			.addEventListener(
				"click",
				() => (State.page = { PROFILE: Setting.profileDrive })
			);
		document.querySelector("#menuSelf img").src =
			Store.files.self[0].avatar ?? Constant.dataDefault.self.avatar;
	});
};
							...
							inputs: [
								{
									...
									events: {
										keyup: (event) => {
											fetch(
												"hyper://" + event.srcElement.value + "/store/self.json"
											)
												.then((result) =>
													result.json().then((json) => {
														extraInfo.innerText = json[0].name;
			...
			},
			{
				...
				events: {
					click: async () => {
						...
						const newDrive = await beaker.hyperdrive.createDrive({
							title: "Kopple",
						});
						window.onhashchange = () => {};
						State.page = "NEWINSTALL";
						Setting.profileDrive = newDrive.url.replace(
							/.*\/\/(.*)\/.*/g,
							"$1"
						);
					},
