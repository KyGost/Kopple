...
class Dialog extends Element {
	constructor(dialog) {
		...
		dialog.inputs.forEach((input) => {
			...
			inputs.push(
				newElement("dialogInputHolder", "div", [
					newElement("dialogLabel", "span", input.label),
					newElement(
						"dialogInput",
						isSelect ? "select" : "input",
						isSelect
							? input.options.map((option) =>
									newElement(undefined, "option", option)
							  )
							: undefined,
						input
					),
		...
		this.element = fromTemplate(DialogTemplate, {
			...
			dialogDone: {
				...
				interactions: {
					click: (element) => {
						(dialog.doneClick || (() => {}))(element), this.remove();
					},
				},
			},
			dialogExtraInfo: {
				appendChildren: [dialog.extraInfo || document.createElement("div")],
			},
